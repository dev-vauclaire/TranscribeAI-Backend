...
from api.Schemas import TranscriptionDiarizationSchema
...
def createDiarizationJob():
    ...
    filename = audio_manager.save_audio(audio_file)
    ...
    return Helpers.success(
        {
            "job_id": job_id,
            "status": "Votre demande de diarization est dans la file d'attente",
        },
        200,
    )
...
def getDiarizationByUuid():
    ...
    if not job or job.type != "DIARIZATION":
        ...
        return Helpers.success(
            {"job_id": job.id, "status": job.status, "position": position}
        )
        ...
        if job.ended_at and job.created_at:
            diarization_duration = (job.ended_at - job.created_at).total_seconds()
