...
from api.Schemas import TranscriptionBatchSchema
...
def createBatchJob():
    ...
    return Helpers.success(
        {
            "job_id": job_id,
            "message": "Votre demande de transcription est dans la file d'attente",
        },
        200,
    )
...
def getBatchTranscriptionByUuid():
    ...
    if not job or job.type != "BATCH":
        ...
        if job.ended_at and job.created_at:
            transcription_duration = (job.ended_at - job.created_at).total_seconds()
