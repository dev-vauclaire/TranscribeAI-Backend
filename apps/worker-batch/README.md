...
`transcription:batch` un par un, tente leur claim PostgreSQL puis délègue le
traitement au transcriber injecté. La boucle conserve les connexions et le
transcriber entre deux messages. Elle ne contient aucune dépendance FastAPI ou
Flask et n'implémente pas encore la finalisation, le retry, la diarization ou le
moteur ML réel.
